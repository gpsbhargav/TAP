...
parser.add_argument('--multiply_sentence_scores', action='store_true')
...
logger = utils.Logger(options.training_log_file)
...
logger.write_log("Reading data pickles")
...
logger.write_log("Training data size:{}".format(len(train_dataset)))
logger.write_log("Dev data size:{}".format(len(dev_dataset)))
...
if options.resume_training or options.dev_only:
    if os.path.isfile(os.path.join(options.save_path,
        ...
        logger.write_log("=> loaded checkpoint. Resuming epoch {}, iteration {}".format(
...
if (options.debugging_short_run):
    logger.write_log("This is a short run")
...
for epoch in range(start_epoch, options.epochs):
    for batch_idx, train_batch in enumerate(train_data_loader):
        ...
        total_loss = (0.000001 * loss_span) +(0.000001 * loss_yes_no_span) 
        ...
        if (lr_this_step < 0):
            logger.write_log("Learning rate < 0. Stopping training")
            break
        ...
        if (torch.isnan(total_loss).item()):
            logger.write_log("Loss became nan in iteration {}. Training stopped".format(
                iterations))
            stop_training_flag = True
            break
        elif (total_loss.item() < 0.0000000000001):
            logger.write_log("Loss is too low. Stopping training")
            stop_training_flag = True
            break
        ...
        if iterations % options.log_every == 0:
            ...
            for threshold in np.linspace(0.1,0.9,5):
                ...
                elif(train_metrics["sp_f1"] >= best_train_metrics["sp_f1"]):
                    best_train_metrics = train_metrics
                    best_train_threshold = threshold
                    best_train_predictions = train_predictions_formatted
            ...
            logger.write_log("Metrics: {}".format(best_train_metrics))
            logger.write_log("Best sf threshold: {}".format(best_train_threshold))
    ...
    for threshold in np.linspace(0.1,0.9,9):
        ...
        elif(dev_metrics["sp_f1"] >= best_dev_metrics["sp_f1"]):
            best_dev_metrics = dev_metrics
            best_dev_threshold = threshold
            best_dev_predictions = dev_predictions_formatted
    
    dev_f1 = best_dev_metrics["sp_f1"]
    ...
    logger.write_log("Yes/no/span EM: {}".format(dev_yes_no_span_em))
    logger.write_log("Official evaluation metrics:")
    logger.write_log("Metrics: {}".format(best_dev_metrics))
    logger.write_log("Best threshold: {}".format(best_dev_threshold))
    logger.write_log('==================================')
    ...
    if (options.debugging_short_run):
        logger.write_log("Short run completed")
        break
